...
{
    public class clsProcesCardlst
    {
         public static List<string> ProcesCardlst { get; set; }
    }
    public partial class ProcessICard : System.Web.UI.Page
    {
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            GridView1.DataSource = studentDetail.Take(500).ToList();
            GridView1.DataBind();
        ...
        {
            SchoolID = Session["SchoolId"].ToString();
            var studentDetail = stdCls.GetStudentDetail(SchoolID).Take(500).ToList();
            if (txttudentName.Text != "")
                studentDetail = studentDetail.Where(a => a.StudentName.ToLower().Contains(txttudentName.Text.ToLower())).ToList();
        ...
        protected void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            if (clsProcesCardlst.ProcesCardlst.Count > 0)
            {
                ...
                Response.Redirect("IDCardProcessTemplate.aspx");
            }
            else
            {
                Page.ClientScript.RegisterStartupScript(GetType(), "msgbox", "alert('Please Select any record for process');", true);
            }
        }
 
        protected void processAll_Click(object sender, EventArgs e)
        {
            SchoolID = Session["SchoolId"].ToString();
